...
GCP_DEPLOY_SSH_PRIVATE_KEY=<고정 OpenSSH 개인키 전체 내용>
```
...
| `GCP_DEPLOY_SSH_PRIVATE_KEY`     | `-----BEGIN OPENSSH PRIVATE KEY-----...`                                                             | 로컬에서 `ssh-keygen -t ed25519 -f gcp_deploy_key -N ""`로 생성 후 개인키 파일 전체 내용을 그대로 등록                  | 매 실행마다 임시 키를 새로 만들고 OS Login 전파를 기다리는 지연을 없애기 위한 고정 키. workflow가 `--ssh-key-file`로 이 키를 지정한다 |

`GCP_DEPLOY_SSH_PRIVATE_KEY`는 워크플로 실행마다 `${{ runner.temp }}/gcp_deploy_ssh_key` 파일로
기록되고, `ssh-keygen -y`로 공개키(`.pub`)를 그 자리에서 만들어낸다. 공개키를 VM이나 GCP
metadata에 미리 등록해 둘 필요는 없다 — OS Login 기반 인증이라 `gcloud compute ssh
--ssh-key-file`이 최초 실행 시 자동으로 배포용 service account의 OS Login 프로필에 공개키를
등록한다. 이후 실행부터는 동일한 키가 이미 등록돼 있으므로 재등록/전파 대기 없이 바로 접속한다.
이 지연 제거가 바로 고정 키를 쓰는 이유다.
