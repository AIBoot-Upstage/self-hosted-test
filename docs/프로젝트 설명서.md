...
`SKILL.md`, 구체적인 증거·오탐 방지 지식 카드와 base 저장소 정책 chunk만 Solar3 prompt에 넣는다.
...
Python 함수의 cyclomatic complexity는 LLM이 추측하지 않는다. 서비스가 base/head 원문을 Radon으로
측정해 전후 수치와 임계값을 제공하고, 모델은 그 근거 위에서 리팩터링 방법만 제안한다.
...
선택 지식 카드가 명시적으로 금지한 주장도 게시 전 규칙으로 제거한다. 예를 들어 assertion 품질
카드로 외부 네트워크나 mock 사용 여부를 추측하거나, 테스트 fake의 placeholder token을 실제
민감정보 유출로 간주하는 리뷰를 허용하지 않는다.
...
정책은 PR head가 아니라 base SHA에서 읽는다. 리뷰 대상 PR이 정책 자체를 함께
변경하더라도 승인 전 정책을 기준으로 검증하기 위해서다. 현재 배포본은 이 공개 온보딩과 저장소
정책 요청 단위 수집은 지원하지만, 공개 온보딩과 정책 영속화·고객별 데이터 격리는 아직 검증되지 않았다.
...
- **Complexity Metric**: 신뢰된 분석 도구가 만든 함수별 변경 전·후 복잡도와 임계값 판정
...
실패 check가 없을 때 자동으로 실행한다. diff 신호로 관련 skill과 base 저장소·공통 정책 유형을 고른 뒤
...
현재 정량 측정은 Python cyclomatic complexity를 지원한다. cognitive complexity와 JavaScript·Go
분석 도구 연결은 후속 범위다.
...
| Radon | Python 복잡도 측정 | LLM 추측 대신 함수별 cyclomatic complexity ground truth 제공 |
...
현재 저장소 정책은 PR base에서 요청 단위로 수집하지만 영속 이력·tenant 격리와 vector 검색까지
완성된 것은 아니므로, “모든 저장소를 자동 학습한다” 또는 “semantic search가 완성됐다”고 설명하지 않는다.
...
- “공식 출처 24개를 27개 증거·오탐 지식 카드로 정제하고 모든 카드의 출처 연결을 검증한다.”
- “Python 심층 리뷰의 cyclomatic complexity는 Radon 전후 측정값과 metric ID를 게시 전 검증한다.”
