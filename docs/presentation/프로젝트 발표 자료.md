...
- Markdown heading 단위 chunk, Postgres 저장, 관련도 상위 3개 사용
...
RAG를 사용한다는 표현만으로 품질을 주장하지 않는다. 정책이 top-3에 실제 포함되는지,
모델이 검색되지 않은 정책을 인용하지 않는지를 Recall@3와 citation validity로 측정한다.
...
| 정책 Retrieval Recall@3 | ≥ 0.85 | baseline pending |
...
4. repository별 정책 수집, source SHA 관리, hybrid retrieval 도입
